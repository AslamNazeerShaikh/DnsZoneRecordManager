namespace DnsZoneRecordManager.Services
{
    ...
    /// (NotFound → 404, anything else → re-rendered form with messages); no string matching on messages.</summary>
    public enum ErrorKind
    {
        /// <summary>Shape validation failed (FluentValidation / DataAnnotations message).</summary>
        Validation = 0,
    ...
    /// <summary>One typed service failure: a kind for control flow plus a guided message for the UI (A4/A5).</summary>
    /// <param name="Kind">Failure kind.</param>
    /// <param name="Message">Human-readable message.</param>
    public sealed record ServiceError(ErrorKind Kind, string Message);

    /// <summary>Outcome of a service operation: data on success, typed errors on failure.
    /// Services never throw for domain failures; unexpected exceptions propagate to the
    /// production exception handler (generic error page, no internals leaked).</summary>
    ...
    public sealed class ServiceResult<T>
    {
        ...
        public static ServiceResult<T> Fail(ErrorKind kind, IEnumerable<string> messages)
        {
            return new ServiceResult<T>(
                default,
                messages.Select(m => new ServiceError(kind, m)).ToList()
            );
        }
